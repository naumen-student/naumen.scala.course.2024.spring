...
object Test2 extends TestSuite {
  override def tests: Tests = Tests {
    ...
    'angleMustSum - (1 to 5).foreach { _ =>
      // В условии написано, что угол может быть только натуральным числом (GradeAngle всегда выражает угол [0, 360).)
      // Поэтому используем модуль (Random.nextInt.abs)
      // Пример: углы 3, -6 и 3
      val angles = Vector.fill(50)(DegreeAngle(Random.nextInt.abs))
      val result = angles.map(_.angel).sum % 360
      println(result)
      println(angles.combineAll)

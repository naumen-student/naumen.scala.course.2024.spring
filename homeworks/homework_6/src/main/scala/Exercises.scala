...
object Exercises {
    ...
    def task1(r: Int, g: Int, b: Int): URIO[ColorService, Option[Color]] = ZIO.serviceWithZIO[ColorService](_.getColor(r, g, b).option)
    ...
    def task4(size: (Int, Int)): IO[GenerationError, Picture] = { task3(size).provideSomeLayer(PictureGenerationService.live).provideLayer(ColorService.live)}

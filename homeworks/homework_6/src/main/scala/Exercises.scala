...
object Exercises {
    ...
    def task2(size: (Int, Int)): ZIO[PictureGenerationService, GenerationError, String] =
        ZIO.serviceWithZIO[PictureGenerationService](_.generatePicture(size).map(_.map(_.getRGB).map(_.toString).mkString("\n")))
    ...
    def task3(size: (Int, Int)): ZIO[PictureGenerationService with ColorService, GenerationError, Picture] =
        for {
            colorServ <- ZIO.service[ColorService].mapError(_ => GenerationError("Не удалось создать цвет"))
            pictureServ <- ZIO.service[PictureGenerationService].mapError(_ => GenerationError("Ошибка генерации изображения"))
            color <- colorServ.generateRandomColor().mapError(_ => GenerationError("Не удалось создать цвет"))
            picture <- pictureServ.generatePicture(size).mapError(_ => GenerationError("Ошибка генерации изображения"))
            filledPicture <- pictureServ.fillPicture(picture, color).mapError(_ => GenerationError("Возникли проблемы при заливке изображения"))
        } yield filledPicture
    ...
    def task4(size: (Int, Int)): IO[GenerationError, Picture] =
        task3(size).provideLayer(PictureGenerationService.live ++ ColorService.live)

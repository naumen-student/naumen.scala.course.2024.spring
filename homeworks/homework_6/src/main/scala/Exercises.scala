...
object Exercises {
     ...
     */
    def task1(r: Int, g: Int, b: Int): URIO[ColorService, Option[Color]] = ZIO
          .serviceWithZIO[ColorService](_.getColor(r, g, b))
          .option
     ...
     */
    def task2(size: (Int, Int)): ZIO[PictureGenerationService, GenerationError, String] = ZIO
      .serviceWithZIO[PictureGenerationService](_.generatePicture(size))
      .map(p => {
            p.lines
              .map(l => l.map(el => -el.getRGB())
              .mkString(" "))
              .mkString("\n")
        })

object Exercises {
    ...
    def divBy3Or7(iFrom: Int, iTo: Int): Seq[Int] = for {
        i <- iFrom to iTo
        if i % 3 == 0 || i % 7 == 0
    } yield i
    ...
    на 3 или на 5.*/
    /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = (iFrom to iTo)
    ...
    def primeFactor(number: Int): Seq[Int] = {
        if (number > 1 && !(2 until number).exists(x => number % x == 0)) Seq(number)
        else {
            var n = number
            var result = Seq[Int]()
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
        ballsArray.mapValues {case (radius, density) => (4 * Math.PI * Math.pow(radius, 3) * density) / 3}
          .toSeq
          .sortBy(_._2)
          .map(_._1)
}
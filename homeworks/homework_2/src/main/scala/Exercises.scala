object Exercises {
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, countFunc:(Vector2D, Vector2D) => Double , rightVec0: Vector2D, rightVec1: Vector2D): Double =
        countFunc(leftVec0, leftVec1) + countFunc(rightVec0, rightVec1)
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        var rad: Int = 0
        var mass: Double = 0
        var l: List[(String, Double)] = List()
        for (ball <- ballsArray) {
            rad = ball._2._1
            mass = 4 / 3 * java.lang.Math.PI * math.pow(rad, 3) * ball._2._2
            l = l :+ (ball._1, mass)
        }
        l = l.sortBy(_._2)
        for (ball <- l) yield ball._1
    }

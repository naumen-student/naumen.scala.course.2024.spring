...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        val multipliers = new ListBuffer[Int]()
        var mutNumber = number
        var potDiv = 2
        while (potDiv * potDiv <= mutNumber) {
            if (mutNumber % potDiv == 0) {
                multipliers += potDiv
            }
            while (mutNumber % potDiv == 0) {
                mutNumber /= potDiv
            }
            potDiv += 1
        }
        if (mutNumber > 1) {
            multipliers += mutNumber
        }
        multipliers.toSeq
    }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toSeq.sortBy{case (_, (radius, density)) => calculateWeight(radius, density)}.map{case (name, (_, _)) => name}
    }

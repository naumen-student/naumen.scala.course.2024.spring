...
object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        @tailrec
        def go(num: Int, divisor: Int = 2, seq: Seq[Int] = Nil): Seq[Int] = {
            if (num == 1) seq
            else if (num % divisor == 0) go(num / divisor, divisor, seq :+ divisor)
            else go(num, divisor + 1, seq)
        }
        go(number).distinct
    }
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, func: Function2[Vector2D, Vector2D, Double],

...
object Exercises extends App {
    ...
    def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = {
        @tailrec
        def go(upperBorder: Int, lowerBorder: Int, accumulator: Long = 0L): Long = {
            if (upperBorder < lowerBorder) accumulator
            else {
                if (upperBorder % 3 == 0 || upperBorder % 5 == 0) go(upperBorder - 1, lowerBorder, accumulator + upperBorder)
                else go(upperBorder - 1, lowerBorder, accumulator)
            }
        }
    ...
    def primeFactors(number: Int): Seq[Int] = {
        @tailrec
        def go(num: Int, divisor: Int = 2, list:Seq[Int]=Nil):Seq[Int] = {
            if (num == 1) list
            else if (num % divisor == 0) go(num / divisor, divisor, list :+ divisor)
            else go(num, divisor + 1, list)
        }
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.toList.map{case(name, (radius, density)) => (name, 4/3 * Math.PI * Math.pow(radius, 3) * density)}

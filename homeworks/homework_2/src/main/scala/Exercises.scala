object Exercises {
    ...
    def primeFactor(number: Int): Seq[Int] = {
        require(number > 0, "Число должно быть положительным")
        ...
        def primeFactorsHelper(n: Int, divisor: Int = 2, acc: Seq[Int] = Seq()): Seq[Int] = {
            if (n <= 1) acc
            else if (n % divisor == 0 && isPrime(divisor)) primeFactorsHelper(n / divisor, divisor + 1, acc :+ divisor)
            else primeFactorsHelper(n, divisor + 1, acc)
        }

        primeFactorsHelper(number)
    }
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, ???, rightVec0: Vector2D, rightVec1: Vector2D) = func(leftVec0, leftVec1) +
      func(rightVec0, rightVec1)
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ballsArray.keys.toSeq.sortBy(material => { val (radius, density) = ballsArray(material)
            density * Math.pow(radius, 3) * Math.PI
        })
    }

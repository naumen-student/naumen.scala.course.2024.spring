...
object Exercises {
    ...
    */
    def primeFactor(number: Int): Seq[Int] = {
      def primeFactorRec(num: Int, cur: Int): mutable.HashSet[Int] = {
        if (num == 0 || num == 1)
          mutable.HashSet[Int]()
        else if (num % cur != 0)
          primeFactorRec(num, cur + 1)
        else primeFactorRec(num / cur, cur) + cur
      }
      primeFactorRec(math.abs(number), 2).toSeq
    }
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, operation: (Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D) = {

...
object Exercises {
    ...
    def findSumFunctional(items: List[Int], sumValue: Int) = {
        items.indices.flatMap { i =>
            items.indices
              .filter(j => i != j && items(i) + items(j) == sumValue)
              .map(j => (i, j))
        }
          .lastOption
          .getOrElse((-1, -1))
    ...
    def tailRecRecursion(items: List[Int]): Int = {
        @tailrec
        def recursion(items: List[Int], index: Int, result: Int): Int =
            items match {
                case head :: tail =>
                    val newResult = if (head % 2 == 0) index + head * result else index - head * result
                    recursion(tail, index - 1, newResult)
                case _ => result
            }
    ...
    def functionalBinarySearch(items: List[Int], value: Int): Option[Int] = {
        ...
        def binarySearch(low: Int, high: Int): Option[Int] = {
            if (low <= high) {
                ...
                items(mid) match {
                    case i if (i == value) => Some(mid)
                    case i if (i < value) => binarySearch(mid + 1, high)
